...
As demonstrated in https://initblog.com/2019/lxd-root/
"""
...
def send_msg(sock_name, msg):
    ...
    # If there is a reply, print it to console
...
Alright, now let let's run the python script again to see if we can now talk to systemd.

```
lowpriv@server:~$ ./systemd_auth.py /run/systemd/private
OK a1cde28f7bdd46f283f348654d7fa07f
AGREE_UNIX_FD
```

Excellent! We are now speaking directly to the host's systemd service as if we are root.

From here, we can continue to analyze `strace` output to build the `systemctl` commands we want to run. While `strace` shows the message data broken into fragments, we can actually piece them together into singe variables in a Python script and thrown them at the socket.
